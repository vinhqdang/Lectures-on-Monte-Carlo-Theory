...
The code below checks, for many random pairs of points in $\R^2$, that
$P_C$ (projection onto the closed unit disk) is simultaneously nonexpansive,
firmly nonexpansive, and that its reflection $S = 2P_C - \Id$ is nonexpansive
(confirming $P_C$ is $\tfrac12$-averaged via Remark~2.1).
...
for _ in range(5000):
    ...
    px, py = P_C(x), P_C(y)
    dp, dxy = px - py, x - y
    if np.linalg.norm(dp) > np.linalg.norm(dxy) + 1e-9:
        ne_ok = False                       # (1) nonexpansive
    if np.dot(dp, dp) > np.dot(dp, dxy) + 1e-9:
        fne_ok = False                      # (2) firmly nonexpansive
    Sx, Sy = 2 * px - x, 2 * py - y          # (3) reflection S=2P_C-Id
    if np.linalg.norm(Sx - Sy) > np.linalg.norm(dxy) + 1e-9:
        avg_ok = False                      #     is nonexpansive?

print("nonexpansive?        ", ne_ok)
print("firmly nonexpansive?  ", fne_ok)
print("reflection nonexpansive (=> P_C averaged)? ", avg_ok)
\end{lstlisting}
...
We confirm $\operatorname{prox}_{\iota_C} = P_C$ by comparing the closed-form
projection formula against a brute-force grid search minimizing $\|y-x\|^2$
over $y \in C$ (the definition of $\operatorname{prox}_{\iota_C}(x)$, since
$\iota_C$ contributes $0$ inside $C$ and $+\infty$ outside).
...
def prox_indicator_bruteforce(x, n_grid=200):
    # brute-force argmin over C of ||y-x||^2 on a polar grid: r in [0,1]
...
closed_form = P_C(x_test)
brute_force = prox_indicator_bruteforce(x_test)
print("closed form P_C(x)  =", closed_form)
print("brute-force prox(x) =", brute_force)
print("agree within grid resolution?",
      np.linalg.norm(closed_form - brute_force) < 0.02)
\end{lstlisting}
...
The two figures used in this deck were generated by \texttt{figures/gen\_figures.py}:
a picture of $P_C$ acting on several points of $\R^2$, and a scatter plot
confirming the firmly-nonexpansive inequality holds (LHS $\le$ RHS) across
...
def project_to_unit_disk(x):
    ...
    return x.copy() if norm <= 1.0 else x / norm
...
for i in range(n_samples):
    ...
    diff_p, diff_xy = px - py, X[i] - Y[i]
    lhs[i] = np.dot(diff_p, diff_p)     # ||P_C(x)-P_C(y)||^2
    rhs[i] = np.dot(diff_p, diff_xy)    # <P_C(x)-P_C(y), x-y>
violations = np.sum(lhs > rhs + 1e-9)
print(f"violations of LHS <= RHS: {violations} / {n_samples}")
# Output: violations of LHS <= RHS: 0 / 4000
\end{lstlisting}
