...
\begin{block}{The Green's function for the BVP (Eq.\ 1.29)}
\[
  G(x,s) = \begin{cases}
...
\begin{frame}[fragile]{Python: Convex Feasibility via the Running Example's VIP}

Checking the VIP characterization (Section 1.2(B$'$)(5)) of $P_C(x_0)$:
...
for _ in range(200_000):
    ...
    worst = max(worst, val)
print(f"x* = {x_star}, max <x0-x*,y-x*> over y in C = {worst:.6f}")
\end{lstlisting}

\medskip
\textbf{Output:} \texttt{x* = [0.6 0.8]}, \texttt{max ... = 0.000000}
(attained only at $y=x^*$) -- confirming $P_C(x_0)$ solves the VIP, hence
the fixed point / convex feasibility problem for $T=P_C$.
