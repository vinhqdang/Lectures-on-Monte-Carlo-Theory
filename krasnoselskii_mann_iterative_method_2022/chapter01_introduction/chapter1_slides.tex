...
# --- Picard iteration on T(x) = -x: oscillates forever ---
x = x0.copy()
picard_trace = [x.copy()]
for _ in range(4):
    x = -x                      # Picard: x_{n+1} = T(x_n)
    picard_trace.append(x.copy())
print("Picard on T=-Id:", [tuple(p) for p in picard_trace])

# --- Krasnoselskii iteration (lambda=1/2) on T = P_C: converges ---
lam = 0.5
x = x0.copy()
km_trace = [x.copy()]
for _ in range(6):
...
\centering
\includegraphics[width=0.92\textwidth]{fig_iteration_comparison.pdf}
...
contraction rate:
\[
  \text{rate}(\lambda) = \sqrt{(1-\lambda)^2+\lambda^2}.
\]
\begin{itemize}
...
\centering
\includegraphics[width=0.62\textwidth]{fig_lambda_rate.pdf}
...
\centering
\includegraphics[width=0.62\textwidth]{fig_projection_feasibility.pdf}
...
# Sample many y in C (the closed unit disk) and check the VIP inequality
...
for _ in range(200_000):
    ...
    worst = max(worst, val)
print(f"max over sampled y in C of <x0-x*, y-x*> = {worst:.6f}  (should be <= 0)")
\end{lstlisting}
