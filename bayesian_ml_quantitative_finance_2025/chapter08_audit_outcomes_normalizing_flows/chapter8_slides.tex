...
\begin{frame}{8.1 Notation Cheat-Sheet for This Chapter}
\begin{itemize}
  \item $D = \{x,y\}$: the data (covariates $x$, binary target $y$); $N$: number of observations.
  \item $\theta$ (or $\mathbf{w}$): the model's parameters; $M$: a specific model (choice of covariates).
  \item $\mathbb{P}(\theta\mid D,M)$, shorthand $\mathbb{P}(\theta_M)$: the \textbf{posterior} -- what MCMC samples for you.
  \item $L(\theta_M) := \mathbb{P}(D\mid\theta,M)$: the \textbf{likelihood}.
  \item $\pi(\theta_M) := \mathbb{P}(\theta\mid M)$: the \textbf{prior}.
  \item $Z_M := \mathbb{P}(D\mid M)$: the \textbf{evidence} -- this chapter's new object of interest.
  \item $B_{ij}$: the \textbf{Bayes factor} comparing model $i$ to model $j$.
\end{itemize}
\vspace{0.2cm}
Keep this slide in mind: every formula from Sect.\ 8.3 onward is built from just these seven symbols.
\end{frame}
...
\begin{frame}{8.2 What Do These Financial Ratios Mean?}
\begin{itemize}
  \item \textbf{Debt to Total Operating Revenue}: how many years of revenue would be needed to pay off all debt -- a leverage/solvency indicator, and the one covariate present in \emph{every} model (it is Model 5).
  \item \textbf{Current Ratio}: current assets $\div$ current liabilities -- a classic short-term liquidity measure.
  \item \textbf{Net Operating Surplus Margin \%}: operating surplus as a \% of revenue -- an operating-efficiency/profitability measure.
  \item \textbf{Capital Expenditure \% Total Expenditure}: how much of total spending goes to capital projects vs.\ day-to-day operations.
  \item \textbf{Debt to Community Wealth/Equity}: leverage relative to the municipality's net asset base.
\end{itemize}
\vspace{0.2cm}
These are exactly the kind of ratios AGSA and researchers (Mabelane et al., 2023) have found most predictive of audit outcomes -- debt ratios, liquidity, and operating-surplus margins consistently rank as top features.
\end{frame}
...
\begin{frame}{8.3 Quick Recap: Posterior vs.\ Evidence}
\begin{itemize}
  \item The \textbf{posterior} $\mathbb{P}(\theta_M) \propto L(\theta_M)\pi(\theta_M)$ answers: \emph{within} model $M$, which parameter values are plausible?
  \item The \textbf{evidence} $Z_M = \int L(\theta_M)\pi(\theta_M)\,d\theta$ answers: \emph{as a whole}, how well does model $M$ explain the data, averaged over everything the prior considers plausible?
  \item MCMC (MALA/HMC/NUTS) gives you the first \emph{for free}; it never computes the second.
  \item Getting $Z_M$ from posterior samples \emph{without any extra sampling scheme} is exactly what the harmonic mean estimator (Sect.\ 8.4) promises to do.
\end{itemize}
\end{frame}
...
\begin{frame}{8.4 Harmonic Mean Estimator: Summary So Far}
\begin{itemize}
  \item \textbf{Identity}: $\mathbb{E}_{\text{posterior}}[1/L(\theta_M)] = 1/Z_M$ -- exact, and derived from Bayes' rule alone.
  \item \textbf{Estimator} (Eq.\ 8.7): average $1/L(\theta^{(i)})$ over posterior samples you already have.
  \item \textbf{Failure mode}: equivalent to importance sampling with the \emph{wrong} tail behaviour (sampling density = posterior, target = prior) -- rare low-likelihood posterior samples can dominate and destabilize the average, in the worst case giving infinite variance.
  \item \textbf{Fix}: replace the (too-broad) prior with a \emph{learned} density whose mass is guaranteed to sit inside the posterior -- this is exactly what a normalizing flow gives us, covered next in Sect.\ 8.4.1.
\end{itemize}
\end{frame}
...
\begin{frame}{8.5.1 Why a Time-Based Train/Test Split?}
\begin{itemize}
  \item The 70--30 split is done \textbf{by year}, not by random shuffling of rows.
  \item This matches the real deployment scenario: a model trained on 2012--$\sim$2016 data is asked to predict audit outcomes for later (2017--2018) municipal-year observations it has never seen.
  \item A random shuffle would let information from a municipality's \emph{future} financial year leak into training, making the reported AUC over-optimistic.
  \item This is exactly the same discipline you would apply when back-testing a trading strategy or a credit model in quantitative finance: never let training data ``see the future'' relative to the test period.
\end{itemize}
\end{frame}
...
\begin{frame}{8.6 What This Means for Practitioners}
\begin{itemize}
  \item Bayesian evidence gives you a \textbf{single, principled number} to rank competing models -- no separate held-out validation set is strictly required to guard against over-fitting (though it is still good practice to check with AUC, as this chapter does).
  \item It correctly rewarded a genuinely useful covariate (Current Ratio, Model $5\to4$) with a very strong Bayes factor, and gave only weak support to a marginally-useful one (Model $2\to1$).
  \item The \textbf{learnt harmonic mean estimator with normalizing flows} makes this practical: no bespoke nested-sampling scheme is needed, and it reuses posterior samples from whichever MCMC sampler (MALA/HMC/NUTS) is already part of the workflow.
  \item This is directly transferable to other quantitative-finance model-selection problems: e.g.\ choosing between competing covariate sets for credit-risk, fraud, or default models (as in Chapters 6, 7 and 9 of this book).
\end{itemize}
\end{frame}
