...
\begin{frame}{Our Toy Running Example: A Synthetic ``Swaption-Like'' Skew}
  ...
  \begin{center}
    \includegraphics[width=0.62\textwidth]{fig_toy_sabr_data.pdf}
  \end{center}
\end{frame}
...
\begin{frame}[fragile]{Python: MALA on the Toy SABR Posterior}
  We infer $(\alpha,\rho)$ from the 9 synthetic noisy swaption-like vols
  introduced earlier, with a Gaussian likelihood and weak Gaussian priors.
  Gradients are computed by finite differences (from scratch, no autodiff).
\begin{lstlisting}
import numpy as np
...
\begin{frame}{Toy MALA Results}
  ...
  and recovers both true parameters closely:
  \begin{center}
    \includegraphics[width=0.85\textwidth]{fig_mala_toy.pdf}
  \end{center}
  Note how correlated consecutive draws look in the trace plots -- each move
  is only a small local nudge, so the chain wanders slowly even though it
  eventually covers the full posterior.
\end{frame}
