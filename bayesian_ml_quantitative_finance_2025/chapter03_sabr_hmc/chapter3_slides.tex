...
\begin{frame}{Our Toy Running Example: A Synthetic ``Swaption-Like'' Skew}
  ...
  \begin{center}
    \includegraphics[width=0.42\textwidth]{fig_toy_sabr_data.pdf}
  \end{center}
\end{frame}
...
\begin{frame}[fragile]{Python: MALA on the Toy SABR Posterior (Log-Posterior)}
...
\begin{frame}[fragile]{Python: MALA on the Toy SABR Posterior (Sampler Step)}
  \small The MALA proposal, drift-adjusted using the gradient above, with the
  matching Metropolis-Hastings correction:
\begin{lstlisting}
...
\end{lstlisting}
  \vspace{2mm}
  \textbf{Illustrative toy result} (our own simulation, not the book's data):
  running this for 6000 iterations (1200 burn-in) with a well-tuned step size
  gives an acceptance rate of about \textbf{68\%} and recovers both true
  parameters closely.
\end{frame}
...
\begin{frame}{Toy MALA Results}
  ...
  Correlated trace plots reflect MALA's small, local proposal nudges; both
  posteriors nonetheless concentrate close to the true parameter values:
  \begin{center}
    \includegraphics[width=0.62\textwidth]{fig_mala_toy.pdf}
  \end{center}
\end{frame}
...
\begin{frame}{HMC: Accept/Reject Step and Full Algorithm}
  ...
  accept ($\Ham$ perfectly conserved); the discretisation error is why this
  MH correction step is needed to keep the chain exact.
  ...
  \begin{enumerate}
    ...
    \begin{enumerate}
      ...
      \item[3.4] $\alpha_m = \min(1, \exp(\delta H))$; \ $u_m \sim \text{Unif}(0,1)$
...
\begin{frame}{Visualising the Leapfrog Trajectory}
  \begin{center}
    \includegraphics[width=0.78\textwidth]{fig_leapfrog_schematic.pdf}
  \end{center}
  \small
...
\begin{frame}[fragile]{Python: HMC on the Toy SABR Posterior}
  \small Applying HMC to the same toy $(\alpha,\rho)$ inference problem as
  MALA, using a diagonal mass matrix (to handle $\alpha$ and $\rho$ living on
  very different natural scales -- $\alpha\!\sim\!0.01$ vs.\ $\rho\!\sim\!1$):
\begin{lstlisting}
...
# to target an 80% acceptance rate (mirrored in our toy demo below).
\end{lstlisting}
\end{frame}
...
\begin{frame}{Toy HMC Results}
  ...
  \begin{center}
    \includegraphics[width=0.58\textwidth]{fig_hmc_toy.pdf}
  \end{center}
\end{frame}
...
\begin{frame}{MALA vs.\ HMC: Why HMC Mixes Faster}
  ...
  \begin{center}
    \includegraphics[width=0.48\textwidth]{fig_mala_vs_hmc_autocorr.pdf}
  \end{center}
  ...
  out to lag $\sim$20 -- more effective samples per iteration.
\end{frame}
