...
% =====================================================================
\begin{frame}{6.3.1 The effective null is the bipolar of $\cP$}
...
% =====================================================================
\begin{frame}{6.3 The RIPr picture and general strong duality}
...
# Finite P = {P1, P2, P3} on a small sample space, Q fixed.
# We solve  min_{lambda in simplex}  KL(Q, sum_k lambda_k P_k)
# to find the RIPr P*, then read off the numeraire E* = dQ/dP*.

P = np.array([[0.6, 0.4],
              [0.3, 0.7],
              [0.5, 0.5]])          # rows P1,P2,P3 over 2 outcomes
Q = np.array([0.2, 0.8])

def kl_objective(raw_lambda):
    w = np.exp(raw_lambda) / np.sum(np.exp(raw_lambda))  # softmax -> simplex
    mixture = w @ P
    return np.sum(Q * np.log(Q / mixture))
...
print("lambda* (RIPr mixture weights):", np.round(lam_star, 4))
print("RIPr P*:                        ", np.round(P_star, 4))
print("numeraire E* = dQ/dP*:          ", np.round(E_star, 4))
print("e-power = E^Q[log E*] = KL(Q,P*):", round(res.fun, 4))

# sanity check: E* must satisfy E^{P_k}[E*] <= 1 for every k
for k in range(3):
    print(f"  E^P{k+1}[E*] = {np.sum(P[k] * E_star):.4f}  (<= 1 ?)")
\end{lstlisting}
\end{frame}
...
% =====================================================================
\begin{frame}{Running example: comparing to universal inference}
...
% =====================================================================
\begin{frame}{Running example: e-power as a general curve}
...
% =====================================================================
\begin{frame}{6.6 The numeraire resolves all of this}
...
# --- 6.7.1 / 6.7.2: exponential family / monotone LR ---
def numeraire_exp_family(theta0, theta1, S, n):
    """One-sided null theta <= theta0, point alt theta1 > theta0."""
    return (theta1/theta0)**S * ((1-theta1)/(1-theta0))**(n-S)

print("Coin fairness numeraire  :", round(numeraire_exp_family(0.5, 0.7, 14, 20), 4))

# --- 6.7.3: testing symmetry, null = symmetric distributions ---
def numeraire_symmetry(qz, qminusz):
    """E* = 2 q(z) / (q(z)+q(-z)) at the observed z."""
    return 2*qz / (qz + qminusz) if (qz+qminusz) > 0 else 0.0

# --- 6.7.4: testing bounded means, null = {E[Z] <= mu}, Z in [0,1] ---
def numeraire_bounded_mean(z, mu, lam_star):
    return 1 + lam_star*(z - mu)

def solve_lambda_star(mu, grid=np.linspace(1e-6, 1/mu - 1e-6, 200000)):
    # first-order condition E^Q[(Z-mu)/(1+lam(Z-mu))] = 0 under Q=Uniform[0,1]
    best_lam, best_abs = None, np.inf
    for lam in grid:
        val = 1 - (1/lam)*np.log((1+lam*(1-mu))/(1-lam*mu))
        if abs(val) < best_abs:
            best_abs, best_lam = abs(val), lam
    return best_lam

# --- 6.7.5: sub-Gaussian means, null = {1-sub-Gaussian, mean <= 0} ---
def numeraire_subgaussian(z, mu):
    return np.exp(mu*z - mu**2/2)     # lambda* = mu, the alternative's mean

# --- 6.7.6: likelihood-ratio bounds, null = {dP/dP0 <= gamma} ---
def numeraire_lr_bound(z_star, z0, gamma):
    return max(z_star, z0) / gamma

print("Sub-Gaussian numeraire at z=1.5, mu=1:", round(numeraire_subgaussian(1.5, 1.0), 4))
\end{lstlisting}
\end{frame}

% =====================================================================
\begin{frame}{6.7.1--6.7.2 Exponential families \& monotone likelihood ratio}
...
% =====================================================================
\begin{frame}{6.7.4 Testing bounded means}
