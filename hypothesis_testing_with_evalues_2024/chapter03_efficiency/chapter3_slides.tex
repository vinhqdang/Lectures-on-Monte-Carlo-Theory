...
\begin{frame}{Running example, part 1: testing a coin's fairness}
  ...
  \begin{block}{Step 1: e-power under the true alternative $\theta_1=0.7$}
    ...
    \]
    This is exactly $\KL{\mathrm{Bern}(0.7)}{\mathrm{Bern}(0.5)} = 0.08228$ nats -- not a coincidence (Section 3.5): the LR e-variable is log-optimal, with e-power always equal to the KL divergence.
  \end{block}
\end{frame}
...
\begin{frame}{Running example, part 2: log-optimality in numbers}
  ...
  \begin{block}{Step 3: compare growth over $n=100$ flips}
    ...
    \emph{The log-optimal e-variable compounds nearly $9\times$ faster over 100 flips}, purely from using the correct likelihood ratio.
  \end{block}
\end{frame}
