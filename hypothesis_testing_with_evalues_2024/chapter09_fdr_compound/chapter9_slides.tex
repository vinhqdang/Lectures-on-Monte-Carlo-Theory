...
\begin{center}
\includegraphics[width=0.6\textwidth]{ebh_illustration.pdf}
\end{center}
\footnotesize Sorted e-values (largest first) vs.\ the e-BH threshold curve $K/(\alpha k)$, $K=20,\alpha=0.1$. Points above the dashed curve are rejected ($k^\ast=4$).
\end{frame}
...
# H_k : X_k ~ N(0,1) (null), K=10. Believed alt. means (pooled/learned):
K = 10
...
# MC check: under the GLOBAL null, E[s(X_k)] <=1 each, sum <= K.
n_mc = 200_000
...
% ============================================================
\begin{frame}[fragile]{Python: minimally adaptive e-BH on our 10 coins (setup)}
\begin{lstlisting}[language=Python]
import numpy as np
...
def ebh_kstar(e_arr, lvl):
    ...
    return k, order
\end{lstlisting}
This helper \texttt{ebh\_kstar} implements the plain e-BH rank/threshold check at any level \texttt{lvl}; we reuse it below both for the initial (level-$\alpha$) and the adaptive (level-$\alpha'$) pass.
\end{frame}

\begin{frame}[fragile]{Python: minimally adaptive e-BH on our 10 coins (result)}
\begin{lstlisting}[language=Python]
...
% ============================================================
\begin{frame}[fragile]{Python: stochastic rounding (setup)}
\begin{lstlisting}[language=Python]
import numpy as np
...
\begin{frame}[fragile]{Python: randomized e-BH (Ge-BH) -- result}
\begin{lstlisting}[language=Python]
def ebh_kstar(e_arr, lvl):
    order = np.argsort(-e_arr)
    e_sorted = e_arr[order]
...
\begin{center}
\includegraphics[width=0.52\textwidth]{fdr_simulation.pdf}
\end{center}
\footnotesize The book's simulations (one-sided Gaussian testing, $K=100$, $\pi_0=0.3$) show De-BH and Ue-BH noticeably improve power over plain e-BH -- especially under negative dependence -- while all three keep FDR below the nominal level, just as our own FDR simulation confirms below.
\end{frame}
