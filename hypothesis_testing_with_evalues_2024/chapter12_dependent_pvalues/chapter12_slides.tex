...
%% =========================================================================
\begin{frame}[allowframebreaks]{Step-by-step: how e-values recover ``twice the average'' (KEY INSIGHT)}
  \begin{block}{Goal}
...
%% =========================================================================
\begin{frame}[allowframebreaks]{Running example, part 1 continued: the e-value route lands in the same place}
...
%% =========================================================================
\begin{frame}[fragile]{Python: 12.1 -- the e-value route, numerically}
  \begin{lstlisting}
import numpy as np
...
print("e-values:", np.round(E, 3))
print("mean(E) =", round(E.mean(), 3), " 1/alpha =", round(1/alpha, 3))
# mean(E) >= 1/alpha  ->  confirms 0.535 is a valid merged p-value
  \end{lstlisting}
\end{frame}
...
# Strong-dependence simulation: K=8 p-values, two anti-dependent halves
...
P = norm.cdf(Z)                                        # each column ~ Unif(0,1)
twice_mean = np.minimum(2 * P.mean(axis=1), 1.0)

for a in [0.05, 0.2, 0.5]:
    emp = (twice_mean <= a).mean()
    print(f"alpha={a:.2f}: empirical P(2*mean<=alpha)={emp:.4f}  (must be <= {a})")
  \end{lstlisting}
\end{frame}
...
print("admissible merged p-value:", round(merged_pvalue(p), 3)) # 0.47  -- sharper!
  \end{lstlisting}
  \small Both are valid p-values here; $0.47$ is the sharper, admissible one.
\end{frame}
...
# use_max_prefix=False: Thm 12.17 (arbitrary dependence, full average).
# use_max_prefix=True : Thm 12.27 (exchangeable, max over running prefixes).
def merged_pvalue(p_row, use_max_prefix):
    ...
    def criterion(eps):
        ...
        if use_max_prefix:
            cums = np.cumsum(vals)
            return np.max(cums / np.arange(1, K+1))  # prefix average, maxed
        return np.mean(vals)
    lo, hi = 1e-12, 1.0
    while criterion(hi) < 1.0 and hi < 1e6:
        hi *= 2.0
    for _ in range(60):                              # bisection for smallest eps
        mid = 0.5*(lo+hi)
        lo, hi = (lo, mid) if criterion(mid) >= 1.0 else (mid, hi)
    return hi

p = np.array([0.02, 0.15, 0.30, 0.60])
print("arbitrary-dependence (Thm 12.17):", round(merged_pvalue(p, False), 3))  # 0.47
print("exchangeable         (Thm 12.27):", round(merged_pvalue(p, True), 3))  # 0.04
  \end{lstlisting}
\end{frame}
...
\begin{frame}{Figure: power gain from exploiting exchangeability}
  \centering
  \includegraphics[width=0.72\linewidth]{fig_exchangeable_power.pdf}\\
