...
\begin{frame}[allowframebreaks]{11.1 Optimal transport: the one-sentence idea}
...
% ---------------------------------------------------------------------------
\begin{frame}[allowframebreaks]{Setup: the operator $\bigoplus$ and the constraint set $D_F$}
...
% ---------------------------------------------------------------------------
\begin{frame}[allowframebreaks]{Lemma 11.1: optimal transport duality}
...
\begin{frame}[allowframebreaks]{11.2 Intuition: what are we proving?}
...
% ---------------------------------------------------------------------------
\begin{frame}[allowframebreaks]{Warm-up: the case $K=1$ (Lemma 11.2)}
...
% ---------------------------------------------------------------------------
\begin{frame}[allowframebreaks]{Consequence of Lemma 11.2: no free lunch from transforming $E$}
...
% ---------------------------------------------------------------------------
\begin{frame}[allowframebreaks]{Upper semicontinuous reduction (Lemma 11.3)}
...
% ---------------------------------------------------------------------------
\begin{frame}[allowframebreaks]{Proof of Theorem 8.4, step 1: set up the transport problem}
...
% ---------------------------------------------------------------------------
\begin{frame}[allowframebreaks]{Proof of Theorem 8.4, step 2: swap sup and inf (minimax)}
...
% ---------------------------------------------------------------------------
\begin{frame}[allowframebreaks]{Proof of Theorem 8.4, step 3: apply Lemma 11.2 coordinatewise}
...
% ---------------------------------------------------------------------------
\begin{frame}[allowframebreaks]{Running numerical example: merging three e-values}
...
% ---------------------------------------------------------------------------
\begin{frame}[allowframebreaks]{Figure: visualizing the merge}
...
def M_lambda(e, lam0, lam):
    # Admissible linear e-merging function: lam0 + sum_k lam_k * e_k,
    # valid whenever lam0 + sum(lam) == 1 and all weights >= 0 (simplex).
    e, lam = np.asarray(e, float), np.asarray(lam, float)
...
M_avg  = M_lambda(E, lam0=0.0, lam=np.full(3, 1/3))   # simple average
M_skep = M_lambda(E, lam0=0.4, lam=np.full(3, 0.2))   # skeptical average
...
\begin{frame}[allowframebreaks]{11.3 Intuition: combining evidence in different currencies}
...
% ---------------------------------------------------------------------------
\begin{frame}[allowframebreaks]{Definition 11.4: combiners}
...
% ---------------------------------------------------------------------------
\begin{frame}[allowframebreaks]{Four natural combiners}
...
% ---------------------------------------------------------------------------
\begin{frame}[allowframebreaks]{Theorem 11.5 and the proof of the [ip] case}
...
% ---------------------------------------------------------------------------
\begin{frame}[allowframebreaks]{The quotient combiner and randomization (Example 11.6)}
...
% ---------------------------------------------------------------------------
\begin{frame}[allowframebreaks]{When to prefer the quotient combiner? (rule of thumb)}
...
\begin{frame}[allowframebreaks]{11.4 Intuition: e-values as unnormalized weights}
...
% ---------------------------------------------------------------------------
\begin{frame}[allowframebreaks]{Definition 11.9: the e-weighted p-value procedure}
...
% ---------------------------------------------------------------------------
\begin{frame}[allowframebreaks]{Theorem 11.10: FDR control for ep-BH}
...
% ---------------------------------------------------------------------------
\begin{frame}[allowframebreaks]{Concrete example: an e-value loosens a threshold}
...
% ---------------------------------------------------------------------------
\begin{frame}[allowframebreaks]{Figure: e-values as weights loosen/tighten thresholds}
...
def ep_bh(p, e, alpha=0.05):
    # E-weighted BH (ep-BH): Definition 11.9 + Theorem 11.10.
    p, e = np.asarray(p, float), np.asarray(e, float)
    K = len(p)
    p_star = np.minimum(p / e, 1.0)            # Step 1: quotient combiner
    order = np.argsort(p_star)                 # Step 2: ordinary BH on p_star
    sorted_p = p_star[order]
    thresh = alpha * (np.arange(1, K + 1) / K)
    below = np.nonzero(sorted_p <= thresh)[0]
    if len(below) == 0:
...
# E_5 = 4.0 loosens hypothesis 5's effective threshold a lot
p = np.array([0.30, 0.02, 0.45, 0.60, 0.18])
e = np.array([1.00, 1.00, 1.00, 1.00, 4.00])
print("effective thresholds:", np.minimum(p / e, 1.0))
...
% ---------------------------------------------------------------------------
\begin{frame}[allowframebreaks]{Bibliographical note}
