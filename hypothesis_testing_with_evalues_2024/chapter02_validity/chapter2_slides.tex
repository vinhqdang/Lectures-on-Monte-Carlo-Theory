...
E_all = e_value(sims)
alpha = 0.193
print("empirical P(E>=1/alpha):", np.mean(E_all >= 1/alpha), " <= alpha?", alpha)
\end{lstlisting}
\end{frame}
...
n, x = 20, 14
theta0 = 0.5
p_val = 1 - stats.binom.cdf(x - 1, n, theta0)     # one-sided p-value
...
# Warning: round-tripping loses evidence -- check with p=0.01
p0 = 0.01
e0 = p0 ** (-0.5) - 1                     # 9.0
p0_back = min(1, 1 / e0)                  # 0.111 (weaker than 0.01!)
...
n, x, theta0 = 20, 14, 0.5
# Simple null {theta0}: Prop. 2.12 says E is admissible iff EXACT,
# i.e. E_theta0[E] == 1.
def e_value_lr(k, theta1):
    loglik = lambda th: k*np.log(th) + (n-k)*np.log(1-th)
    return np.exp(loglik(theta1) - loglik(theta0))

theta1 = x / n                    # 0.7, MLE alternative used throughout
k = np.arange(n + 1)
pmf0 = binom.pmf(k, n, theta0)
E_k = e_value_lr(k, theta1)
mean_E = np.sum(pmf0 * E_k)
print(f"E_theta0[E] = {mean_E:.6f}")   # 1.000000 => exact => admissible

# Contrast: mean-based e-variable from Sec. 2.6.3, (1-lam)+lam*Z/mu,
# lam=1, Z=heads, mu=10 -- admissible via Corollary 2.15(b) though not
# exact in general (only exact at the boundary case Z=mu a.s.)
mu = n * theta0
mean_E_mean = np.sum(pmf0 * (k / mu))
print(f"E_theta0[Z/mu] = {mean_E_mean:.6f}")   # = 1.0 for this theta0
\end{lstlisting}
\end{frame}
...
# Sec. 2.6.3: distribution-free mean-based e-variable, lambda=1
E_mean = x / mu
...
# Sec. 2.6.2 flavor: sign-based e-value, k = #heads = #"+1" signs
# (signs i.i.d. Uniform{-1,1} under H0: fair coin), candidate bias p
k, p_cand = x, 0.7
E_signs = (p_cand**k * (1 - p_cand)**(n - k)) / (2**-n)
print(f"sign-based e-value at p={p_cand} = {E_signs:.3f}")
# E_lr and E_signs coincide: both reduce to the same binomial
...
\begin{frame}{Chapter 2 Summary}
  \begin{block}{What We Learned}
    \begin{itemize}
      \item \textbf{Markov's inequality} (Prop.\ 2.1): $1/E$ is always a
        p-variable -- the universal, if conservative, bridge from any
        e-value to a level-$\alpha$ test.
      \item Knowing the \textbf{exact null distribution} of $E$ (Sec.~2.2)
        gives sharper thresholds than $1/\alpha$.
      \item \textbf{Calibrators} (Sec.~2.3): the e-to-p direction has a
        \emph{unique} admissible calibrator $\min(1,1/t)$; the p-to-e
        direction admits a rich family.
      \item \textbf{External randomization} (Sec.~2.4) strictly improves
        on Markov's inequality (Theorem 2.9).
      \item \textbf{Admissible e-variables} (Sec.~2.5) cannot be enlarged;
        for simple nulls they are exactly the likelihood ratios.
      \item Sec.~2.6 catalogued e-variables for MLR families, symmetry,
        means/variances, sub-Gaussian means, constrained hypotheses,
        likelihood-ratio bounds, and exchangeability.
      \item Sec.~2.7's Jeffreys-style grid interprets a realized e-value's
        strength -- but e-values and p-values need not agree in
        \emph{direction}.
      \item \textbf{Running example}: our coin ($n=20$, 14 heads) gave a
        likelihood-ratio e-value $\approx5.18$ and a p-value
        $\approx0.058$, calibrated between both directions throughout.
    \end{itemize}
  \end{block}
\end{frame}
