...
def empirical_var(losses, beta):
    """VaR_beta(F) = inf{x : F(x) >= beta}."""
    x = np.sort(losses)
    n = len(x)
    k = min(max(int(np.ceil(beta * n)), 1), n)
    return x[k - 1]

def empirical_es(losses, beta):
    """ES_beta(F), exact tail integral for the empirical n-point law."""
    x = np.sort(losses)
    n = len(x)
    total, k = 0.0, int(np.ceil(beta * n))
    for i in range(k, n + 1):
        total += (i / n - max(beta, (i - 1) / n)) * x[i - 1]
    return total / (1 - beta)

np.random.seed(1)
losses = np.random.normal(loc=0.5, scale=2.0, size=100_000)  # simulated losses
...
# Direct definition: ES_beta(F) via order statistics
x_sorted = np.sort(X)
k = int(np.ceil(beta * n))
es_direct = x_sorted[k - 1:].mean()                 # average of the worst tail

# Dual side: optimal P* puts likelihood ratio `cap` on the top (1-beta)
# fraction of X, 0 elsewhere -- approximate on our empirical sample:
weights = np.zeros(n)
weights[np.argsort(X)[-(n - k + 1):]] = cap / n
es_dual = np.sum(weights * X) / np.sum(weights)     # E^{P*}[X], dP*/dP0 <= cap
...
\textbf{Interpretation.} $M_{10}\approx 9.04 > 1$ is evidence against $H_0$: the observed exceedance rate ($30\%$) is far above the $5\%$ rate expected under $\mathrm{VaR}_{0.95}(F)\le 3$. (Ville's inequality: this e-process bounds the anytime-valid p-value by $1/9.04\approx 0.111$.)
...
\begin{center}
\includegraphics[width=0.62\textwidth]{fig2_eprocess.pdf}
\end{center}
\vspace{-2mm}
{\footnotesize Top: the 10 daily losses, exceedances of threshold $r=3$ in red. Bottom: e-process $M_t=\prod_{s\le t}(0.5+0.5E_s)$ testing $H_0:\mathrm{VaR}_{0.95}(F)\le 3$. Exceedances multiply $M_t$ by $10.5$; non-exceedances by $0.5$. Ends at $M_{10}\approx 9.04$, evidence against $H_0$.}
