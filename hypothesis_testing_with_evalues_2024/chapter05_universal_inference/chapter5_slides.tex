...
%--------------------------------------------------------------
\begin{frame}[allowframebreaks]{Theorem 5.1: validity of the split LR e-variable}
...
\begin{frame}{Running example: Gaussian vs.\ Gaussian mixture}
  ...
  \small
  \textbf{Setup.} $H_0$: $N(\mu,1)$ (one unknown mean). $H_1$:
  2-component mixture $(1{-}\lambda)N(\mu_1,1)+\lambda N(\mu_2,1)$,
  $\mu_1,\mu_2,\lambda$ all free (the irregular model of Sec.\ 5.1).
  We simulate $n=40$ points from a genuine mixture, split $20/20$ into
  $D_0, D_1$.
  ...
  \begin{columns}
    \begin{column}{0.55\textwidth}
      \begin{block}{What we computed (one concrete run)}
        \small
        \begin{itemize}
          \item Null MLE on $D_0$ (sample mean, var.\ known $=1$):
            $\hat\mu_0 = 0.289$.
          \item Alt.\ fit on $D_1$ (2-comp.\ mixture MLE, numerical opt.):
        ...
        \end{itemize}
      \end{block}
      Since $E = 285.8 \gg 1/\alpha = 20$ at $\alpha=0.05$, the split
      LRT rejects $H_0$ here -- correctly, since the data are a mixture.
    \end{column}
...
%--------------------------------------------------------------
\begin{frame}[fragile]{Python: split LR e-variable (5.2) -- part I}
...
%--------------------------------------------------------------
\begin{frame}[fragile]{Python: split LR e-variable (5.2) -- part II}

  Generating the $n=40$ toy dataset, splitting it $20/20$, and calling
  the function defined on the previous slide:
...
%--------------------------------------------------------------
\begin{frame}[fragile]{Python: subsampled / cross-fit LR e-variable (5.3) -- part I}
...
%--------------------------------------------------------------
\begin{frame}[fragile]{Python: subsampled / cross-fit LR e-variable (5.3) -- part II}

  Wrapping the split-LR function from Section 5.2 and running the
  sequential subsampled LRT on the toy dataset:
...
%--------------------------------------------------------------
\begin{frame}[allowframebreaks]{Fact 5.5 and Theorem 5.6 (EMI)}
...
%--------------------------------------------------------------
\begin{frame}[allowframebreaks]{Turning arbitrary dependence into exchangeability}
...
%--------------------------------------------------------------
\begin{frame}[allowframebreaks]{Constructing the universal confidence set}
...
%--------------------------------------------------------------
\begin{frame}[allowframebreaks]{5.6.2 Extension: smoothed likelihood}
...
%--------------------------------------------------------------
\begin{frame}[allowframebreaks]{5.6.3 Extension: relaxed, powered, and conditional likelihood}
...
%--------------------------------------------------------------
\begin{frame}[fragile]{Python: our own reproduction (5.7) -- part I}
...
%--------------------------------------------------------------
\begin{frame}[fragile]{Python: our own reproduction (5.7) -- part II}

  Estimating power at a given $n$ by Monte Carlo: repeatedly simulate
  data, run $B=3$ independent splits, and check both the plain
  split-LRT ($E^{(1)}$ alone) and the subsampled LRT (average of the
  $B$ e-variables) against the threshold $1/\alpha$.
