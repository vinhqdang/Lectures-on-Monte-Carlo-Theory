...
\begin{frame}[allowframebreaks]{12.2 \; Basic Functionalities of VRP Routing Software}
  ...
  \begin{figure}
    ...
    \caption{Architecture of a typical VRP routing software system.
             Data flows from inputs through the GIS and problem-data modules
             into the routing engine, and results are delivered as routes,
             maps, and performance reports.}
  \end{figure}
...
\begin{frame}[allowframebreaks]{12.4 \; Model Properties}
  ...
  \begin{block}{12.4.1 \; Basic setup}
  ...
  \begin{block}{12.4.2 \; Constraint-Related Properties}
  ...
  \begin{block}{Route Start/End \& Same-Tour Constraints}
  ...
  \begin{figure}
    ...
    \caption{Left: two optimised routes for 8 customers, each visiting a
             sequence of customers and returning to the depot.
  ...
  \begin{block}{Capacity Constraints}
    ...
    maximum capacity $Q$ (Q, a positive integer or real number representing
  ...
  \begin{block}{Driving Regulations}
    ...
    breaks at feasible locations (e.g., customer service stops or
    designated rest areas) on each route.
  ...
  \begin{figure}
    \centering
    \includegraphics[width=0.75\textwidth]{fig_model_properties.pdf}
    \caption{Schematic comparison of model property coverage between
             ...
             constraint types than open-source tools (orange),
             particularly for heterogeneous fleets, pickup-and-delivery,
...
\begin{frame}[allowframebreaks]{12.5 \; Algorithms in Commercial VRP Software}
  ...
  \begin{figure}
    \centering
    \includegraphics[width=0.90\textwidth]{fig_algorithm_landscape.pdf}
  ...
  \begin{figure}
    \centering
    \includegraphics[width=0.90\textwidth]{fig_clarke_wright.pdf}
    \caption{Clarke-Wright algorithm illustration.
             ...
             The savings table (top right) shows the computed $s(i,j)$ values.}
  \end{figure}

  \framebreak

  \begin{block}{Clarke-Wright: Pseudocode}
    \textbf{Algorithm: Clarke-Wright Savings (parallel version)}
    \begin{enumerate}
      ...
      \item \textbf{Merge greedily}: for each pair $(i,j)$ in sorted order:
        \begin{enumerate}
          ...
          \item[4b.] Check if merging does not violate the capacity constraint:
                     combined load $\leq Q$.
...
\begin{frame}[fragile]{Clarke-Wright: Python Implementation}
...
def clarke_wright(depot, customers, demands, capacity):
    ...
    # Step 1: compute distance helper
    def dist(a, b):
        return np.linalg.norm(np.array(a) - np.array(b))

    # Step 2: compute savings s(i,j)
    savings = []
    for i, j in combinations(ids, 2):
        s = dist(depot, customers[i]) + dist(depot, customers[j]) \
            - dist(customers[i], customers[j])
        savings.append((s, i, j))
    savings.sort(reverse=True)  # descending order

    # Step 3: initialise individual routes
    routes = {i: [i] for i in ids}  # customer -> route list
    ...
    for s, i, j in savings:
        ...
        # i must be at end of its route, j at start
        ri_route = routes[ri]
        rj_route = routes[rj]
...
\end{frame}
  ...
  \begin{frame}[allowframebreaks]{Metaheuristics in VRP Software}

  After a construction heuristic produces an initial solution, metaheuristics
  are used to improve it.
  The most common metaheuristic in commercial VRP software is
  \textbf{Tabu Search}.
    ...
    \begin{enumerate}
      ...
      \item Define a \textbf{neighbourhood} $\mathcal{N}(s)$: the set of
            ...
            \begin{itemize}
              ...
              \item \emph{2-opt}: remove two edges and reconnect differently
                    (reverses a segment of a route).
              \item \emph{Or-opt}: move a sequence of 1, 2, or 3 consecutive
                    customers to a new position.
  ...
  \begin{block}{ALNS: Adaptive Large Neighbourhood Search}
    ...
    This allows the algorithm to automatically focus on the most effective
    operators for the given instance.

    Typical destroy operators:
    \begin{itemize}
      \item \emph{Random removal}: remove $q$ randomly chosen customers.
      \item \emph{Related removal}: remove $q$ customers that are geographically
            close or serve similar routes.
  ...
  \begin{block}{Multi-threading and parallel computing}
    Most software is built for Microsoft Windows (some for Linux), but
    macOS support is rare.
  ...
  \begin{figure}
    \centering
    \includegraphics[width=0.82\textwidth]{fig_performance_vs_size.pdf}
...
\begin{frame}[allowframebreaks]{12.7 \; VRP Technology Survey}
  ...
  \begin{figure}
    \centering
    \includegraphics[width=0.90\textwidth]{fig_vendor_table.pdf}
  ...
  \begin{block}{12.7.1 \; System Architecture and Service Interfaces}
    ...
    \begin{itemize}
      ...
      \item \textbf{API / web service} ---
            ...
            This is increasingly important for system integration.
  ...
  \begin{block}{12.7.4 \; Vehicle Routing Research and Academia}
    ...
    \begin{itemize}
      ...
      \item Several vendors mentioned that academic VRP tools
            (e.g., research prototype implementations) are rarely directly usable
...
\begin{frame}[allowframebreaks]{12.8 \; New and Emerging Technologies}
  ...
  \begin{figure}
    \centering
    \includegraphics[width=0.82\textwidth]{fig_emerging_technologies.pdf}
  ...
  \begin{block}{GPS and telematics}
    ...
    \begin{itemize}
      ...
      \item refrigeration unit temperature (for cold chain),
  ...
  \begin{figure}
    \centering
    \includegraphics[width=0.85\textwidth]{fig_realtime_loop.pdf}
  ...
  \begin{block}{Cloud computing and SaaS}
    ...
    \begin{itemize}
      ...
      \item \textbf{Scalability on demand}: a cloud-based solver can spin
            ...
            and release them when done --- the customer pays only for
            what they use.
  ...
  \begin{figure}
    \centering
    \includegraphics[width=0.88\textwidth]{fig_saas_vs_onpremise.pdf}
...
\begin{frame}[allowframebreaks]{12.9 \; The Future of the VRP Business}
  ...
  \begin{block}{Vehicles for vehicle routing}
  ...
  \begin{block}{Selected Bibliography (key references from Chapter 12)}
    ...
    \begin{itemize}
      ...
      \item \textbf{Desaulniers, Desrosiers \& Solomon (2002)}:
            ...
            --- Column generation for exact VRP.
...
% ====================================================================
%  APPENDIX: Reference Python Implementation — ALNS for CVRP
% ====================================================================
\appendix

\begin{frame}[fragile]{Appendix A: ALNS Skeleton for CVRP in Python}
  \begin{lstlisting}[caption={Adaptive Large Neighbourhood Search (ALNS) for CVRP}]
...
def alns_cvrp(depot, customers, demands, capacity,
    ...
    depot     : index 0 location
    customers : list of customer indices (1..n)
    demands   : dict {customer: demand}
    capacity  : vehicle capacity Q
    n_iter    : number of iterations
    q         : number of customers to remove per destroy
    T0        : initial temperature (for simulated annealing acceptance)
    alpha     : cooling rate (alpha, alpha) — temperature decreases each step
    """
    ...
    def greedy_solution():
        ...
        while unvisited:
            ...
            while unvisited:
                ...
                for c in unvisited:
                    d = dist(cur, c)
                    if load + demands[c] <= capacity and d < bd:
                        best, bd = c, d
                if best is None:
                    break
                route.append(best)
    ...
    def destroy(routes, q):
        ...
        to_remove = random.sample(flat, min(q, len(flat)))
        new_routes = [[c for c in r if c not in to_remove]
                      for r in routes]
        new_routes = [r for r in new_routes if r]
        return new_routes, to_remove
        ...
        for c in removed:
            ...
            for ri, r in enumerate(routes):
                for pos in range(len(r) + 1):
                    ...
                    if delta < best_cost:
                        best_cost, best_r, best_pos = delta, ri, pos
            if best_r >= 0:
                routes[best_r].insert(best_pos, c)
            else:
                routes.append([c])  # new route
        return routes

    # --- Main ALNS loop ---
    sol = greedy_solution()
    best_sol = copy.deepcopy(sol)
    best_cost = route_cost(sol)
    T = T0
...
\begin{frame}[fragile]{Appendix B: 2-opt Local Search for VRP}
...
def two_opt(route, dist_matrix):
    ...
    A 2-opt move removes two edges (i, i+1) and (j, j+1) and
    reconnects as (i, j) and (i+1, j+1), effectively reversing
    the segment between positions i+1 and j.
    """
    def route_cost(r):
        # includes depot (index 0) at start and end
        full = [0] + r + [0]
    ...
    while improved:
        ...
        for i in range(len(best) - 1):
            for j in range(i + 2, len(best)):
                # try reversing best[i+1 : j+1]
                candidate = best[:i+1] + best[i+1:j+1][::-1] + best[j+1:]
                if route_cost(candidate) < route_cost(best) - 1e-10:
                    best = candidate
                    improved = True  # keep searching
    return best

# --- Example usage ---
if __name__ == '__main__':
    # Small example: 5 customers, random symmetric distance matrix
    np.random.seed(0)
    ...
    improved_route = two_opt(initial_route, D)
    print("Initial route:", initial_route)
    print("Improved route:", improved_route)
    print("Cost before:", sum(D[[0]+initial_route+[0]][:-1,1:].diagonal()))
  \end{lstlisting}
\end{frame}
